...
Copied from Fluent Python GitHub:
https://github.com/fluentpython/example-code-2e/blob/master/20-executors/getflags/flags2_common.py
"""
...
POP20_CC = (
    'CN IN US ID BR PK NG BD RU JP '  # noqa: SIM905
...
def save_flag(img: bytes, filename: str) -> None:  # noqa: D103
    (DEST_DIR / filename).write_bytes(img)


def initial_report(
    cc_list: list[str],  # noqa: D103
    actual_req: int,
    server_label: str,
) -> None:
    if len(cc_list) <= 10:
...
def expand_cc_args(
    every_cc: bool,  # noqa: D103
    all_cc: bool,
    cc_args: list[str],
    limit: int,
) -> list[str]:
    codes: set[str] = set()
    A_Z = string.ascii_uppercase  # noqa: N806
    if every_cc:
        ...
        for cc in (c.upper() for c in cc_args):
            if len(cc) == 1 and cc in A_Z:
                ...
                raise ValueError(
                    '*** Usage error: each CC argument '
                    'must be A to Z or AA to ZZ.'
                )
    return sorted(codes)[:limit]


def process_args(default_concur_req):  # noqa: D103
    ...
    if args.server not in SERVERS:
        print(
            f'*** Usage error: --server LABEL must be one of {server_options}'
        )
        parser.print_usage()
        sys.exit(2)  # command line usage error
    try:
...
def main(download_many, default_concur_req, max_concur_req):  # noqa: D103
    ...
    final_report(cc_list, counter, t0)  # type: ignore
